...
try {
    ...
    $company = [
        'name'   => $settings['brand_name'] ?? 'Kryptosuchmaschine GmbH',
        'addr1'  => $settings['company_addr1'] ?? 'Beispielstraße 123',
        'addr2'  => $settings['company_addr2'] ?? '10115 Berlin, Deutschland',
        'reg'    => $settings['company_reg']   ?? 'HRB 123456 • USt-ID: DE123456789',
    ...
    $y = $pdf->gridRow([['Servicegebühr (5%)',120,'L'],       [$pdf->money($D['service_fee']),50,'R']], 20, $y, 9, true);
    $y += GAP_LG;
    ...
    $pdf->SetFont('Helvetica','',11);
    $pdf->MultiCell(170, 6, pdf_txt(
        "Die Servicegebühr in Höhe von 5 % der Rückerstattungssumme ist vor Freigabe der Auszahlung vollständig zu entrichten. ".
    ...
    $pdf->SetFont('Helvetica','',11);
    $pdf->MultiCell(170, 6, pdf_txt(
        "Die Kryptosuchmaschine GmbH haftet ausschließlich für die ordnungsgemäße Durchführung der Auszahlung auf ".
        "das vom Kunden angegebene Konto, sofern die Servicegebühr vollständig eingegangen ist. Für Verzögerungen ".
        "oder Fehler, die durch Drittbanken oder Zahlungsanbieter entstehen, übernimmt die Kryptosuchmaschine GmbH keine Haftung."
    ), 0, 'L');
    ...
    $pdf->SetFont('Helvetica','',11);
    $pdf->MultiCell(170, 6, pdf_txt(
        "Für sämtliche Streitigkeiten aus diesem Vertragsverhältnis ist der Sitz der Kryptosuchmaschine GmbH in Berlin ".
